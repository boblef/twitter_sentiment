...
class Config(object):
    ...
    def __init__(self,
        ...
        bert_model: BertModel
            The BERT model to be used. For example:
        ...
        warm_up_proportion: float
            During the training, the learning rate is linearly increased.
             This value determines when the learning rate
            reaches the intended learning rate. Default value is 0.1.
        no_cuda: bool
            Determines whether to use gpu. Default is False.
        do_lower_case: bool
            Determines whether to make all training
            and evaluation examples lower case. Default is True.
        seed: int
            Random seed. Defaults to 42.
        local_rank: int
            Used for number of gpu's that will be utilized.
             If set -1, no distributed training will be done. Default
            value is -1.
        ...
        gradual_unfreeze: bool
            Determines whether to gradually unfreeze lower and lower layers
             as the training goes on.
        encoder_no: int
            Starting from which layer the model is going to be finetuned.
            If set 12, whole model is going to be
            fine-tuned. If set, for example, 6,
            only the last 6 layers will be fine-tuned.
        """
...
class FinBert(object):
    ...
    def prepare_model(self, label_list):
        """
        Sets some of the components of the model: Dataset processor,
        number of labels, usage of gpu and distributed
        training, gradient accumulation steps and tokenizer.

        Parameters
        ----------
        label_list: list
            The list of labels values in the dataset.
        ...
        if os.path.exists(self.config.model_dir) \
                and os.listdir(self.config.model_dir):
    ...
    def get_data(self, phase):
        ...
        examples: list
            ...
            Each InputExample is an object that includes the information.
            example;
            text, id, label...
        """
        ...
        self.num_train_optimization_steps = int(
            len(examples) / self.config.train_batch_size /
            self.config.gradient_accumulation_steps) *\
    ...
    def create_the_model(self):
        ...
        if self.config.discriminate:
            # apply the discriminative fine-tuning.
            # discrimination rate is governed by dft_rate.

            encoder_params = []
            for i in range(12):
                encoder_decay = {
                    'params': [p for n, p in list(model.bert.encoder.layer[i]
                                                  .named_parameters())
            ...
            optimizer_grouped_parameters = [
                {'params': [p for n, p in list(model.bert.embeddings
                                               .named_parameters())
                            if not any(nd in n for nd in no_decay)],
                    'weight_decay': 0.01,
                    'lr': lr / (dft_rate ** 13)},
                {'params': [p for n, p in list(model.bert.embeddings
                    ...
                    'weight_decay': 0.0,
                 'lr': lr / (dft_rate ** 13)},
                {'params': [p for n, p in list(model.bert.pooler
                                               .named_parameters())
                            if not any(nd in n for nd in no_decay)],
                    'weight_decay': 0.01,
                    'lr': lr},
                {'params': [p for n, p in list(model.bert.pooler
                    ...
                    'weight_decay': 0.0,
                    'lr': lr},
                {'params': [p for n, p in list(model.classifier
                                               .named_parameters())
                            if not any(nd in n for nd in no_decay)],
                    'weight_decay': 0.01,
                 'lr': lr},
    ...
    def get_loader(self, examples, phase):
        ...
        phase: 'train' or 'eval'
            Determines whether to use random sampling or
             sequential sampling depending on the phase.
    ...
    def train(self, train_examples, model):
        ...
        for _ in trange(int(self.config.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader,
                ...
                if (self.config.gradual_unfreeze and i >
                ...
                if (step + 1) % self.config.gradient_accumulation_steps == 0:
                    if self.config.fp16:
                        lr_this_step = \
                            self.config.learning_rate * warmup_linear(
            ...
            for input_ids, input_mask, segment_ids, label_ids, agree_ids in\
                    tqdm(validation_loader, desc="Validating"):
    ...
    def evaluate(self, model, examples):
        ...
        evaluation_df: pd.DataFrame
            ...
            for each example predicted probability and labels.
        """
        ...
        evaluation_df = pd.DataFrame({'predictions': predictions,
                                      'labels': labels,
...
def predict(text, model):
    ...
    The function first tokenizes sentences, make predictions and write
    results.
    ...
    print("Prediction Result: ", batch_result)
    print("###############")
    return batch_result
