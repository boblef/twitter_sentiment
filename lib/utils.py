...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask,
                 segment_ids, label_id, agree=None):
...
class FinSentProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, phase):
        ...
        return self._create_examples(self._read_tsv(
            os.path.join(data_dir, (phase + ".csv"))), phase)
...
def convert_examples_to_features(examples, label_list, max_seq_length,
                                 tokenizer, mode='classification'):
    """
    Loads a data file into a list of InputBatch's. With this function,
    the InputExample's are converted to features
    that can be used for the model. Text is tokenized,
    converted to ids and zero-padded. Labels are mapped to integers.
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if mode == 'classification':
            ...
            raise ValueError(
                "The mode should either be classification or regression.\
                     You entered: " + mode)
...
def get_metrics(df):
    ...
    df.loc[:, 'accurate'] = df.apply(
        lambda x: x['guess'] == x['labels'], axis=1)
    accuracy = df.accurate.sum() / df.shape[0]

    pos_recall = df[df['labels'] == 0].accurate.sum() / \
        df[df['labels'] == 0].shape[0]
    neg_recall = df[df['labels'] == 1].accurate.sum() / \
        df[df['labels'] == 1].shape[0]
    net_recall = df[df['labels'] == 2].accurate.sum() / \
        df[df['labels'] == 2].shape[0]

    pos_precision = df[df['guess'] == 0].accurate.sum() / \
        df[df['guess'] == 0].shape[0]
    neg_precision = df[df['guess'] == 1].accurate.sum() / \
        df[df['guess'] == 1].shape[0]
    net_precision = df[df['guess'] == 2].accurate.sum() / \
        df[df['guess'] == 2].shape[0]

    pos_f1score = 2 * (pos_precision * pos_recall) / \
        (pos_precision + pos_recall)
    neg_f1score = 2 * (neg_precision * neg_recall) / \
        (neg_precision + neg_recall)
    net_f1score = 2 * (net_precision * net_recall) / \
        (net_precision + net_recall)

    return {'Accuracy': accuracy,
            'Positive': {'precision': pos_precision,
                         'recall': pos_recall,
                         'f1-score': pos_f1score},
            'Negative': {'precision': neg_precision,
                         'recall': neg_recall,
                         'f1-score': neg_f1score},
            'Neutral': {'precision': net_precision,
                        'recall': net_recall,
                        'f1-score': net_f1score}}
...
def get_prediction(text, model, tokenizer):
    ...
    model.eval()
    prediction = softmax(
        model(all_input_ids, all_segment_ids, all_input_mask).detach().numpy())
    return prediction
